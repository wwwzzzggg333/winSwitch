...
| 行 | 内容来源 |
|---|---|
| 应用版本 | `WINSWITCH_VERSION` 编译宏（见 7.2.4） |
...
同时将 `project(winSwitch VERSION 0.1.0 ...)` 升为 `0.2.0`。
...
| `CMakeLists.txt` | 版本升 0.2.0；`WINSWITCH_VERSION` 宏 |
...
- **导出**：先把 UI 当前状态收集为 `Config`（把 `saveFromUi()` 里 73–97 行的收集逻辑抽成 `Config collectFromUi() const`，`saveFromUi` 与导出共用），再 `QFileDialog::getSaveFileName`（过滤器 `*.json`，默认文件名 `winSwitch-config.json`），调用 `exportTo`。失败 `QMessageBox::warning`，成功 `QMessageBox::information`。
